...
bool TwoSegmentsIntersects(vec2 a_1, vec2 a_2, vec2 b_1, vec2 b_2) {//vizsgálja hogy két szakasz metszi-e egymást
	...
	/*
	* elv:
	* Elsőnek megvizsgáljuk hogy az x tengelyen lévő intervallumaiknak van-e közös része. Ha nincs akkor nem metszehtik egymást, ha van megyünk tovább
	* Felírjuk a két vektorra illesztett egyenes egyenletét és ha azok kiegyenlítik egymást akkor van metsző pont aminek megvannak a koordinátái
	* Megnézzük, hogy a metsző pont x koordinátája eleme e a két intervallum közös részének
...
int Intersects(int arr[122], vec2 vert[50]) {//visszaadja hogy hány metsző él van
	int sum = 0;
...
public:
	...
	void GetOptimalSet() {
		...
		for (int i = 0; i < 5000; i++) {//100 grafot generálunk
			...
			//100 esetből kiválasztjuk azt amelyikben a legkevesebb metsző él van
			if ((i - (50 * (i / 50))) == 49) {
	...
	void DrawCircles() {
		...
		for (int i = 0; i < 50; i++) {
			//elsőnek kirajzoljuk a kört
			gpuProgram.setUniform(0, "isTexture");//color színű legyen a pixel
			Circle(i);
...
void ZH() {
	...
	float dist1 = acosh(-lorentz(m1, Point));//m1 t távolság
	...
	vec3 v1 = (m1 - (Point * cosh(dist1))) / sinh(dist1);//irányvektor t pontban
	printf("iranyvektor P pontban %f %f %f\n", v1.x, v1.y, v1.z);
	vec3 t1 = (Point * cosh(2 * dist1)) + (v1 * sinh(dist1 * 2));//t tükrözve m1-re
...
void Moving(vec2 MousePos) {
	...
	for (int i = 0; i < 50; i++) {
		vec3 t = graph->vertices3D[i];//csak azért hogy kevesebbet kelljen írni a későbiekben
...
vec2 ForceBetweenNeighbors(vec2 a, vec2 b, float param, float dist) {//ha szomszédok ez a kettő pont között az erő
	vec2 temp = ((a - b) * (log(param)));// = log(d/d*)<-úgy jöttem, rá, hogy ez a fv hasonlít a legjobban a házi videóban lévő függényre
	return temp;
}

vec2 ForceBetweenVerts(vec2 a, vec2 b, float param, float dist) {//ha nem szomszédok ez a két pont között az erő
	vec2 temp = ((a - b) * ((-1 / (param * param))));// -1/(d / d*)^2 < -úgy jöttem, rá, hogy ez a fv hasonlít a legjobban a házi videóban lévő függényre
	return temp;
}
...
void Sorting() {
	...
	for (int i = 0; i <50; i++) {
		...
		vec2 force;//eredő erő egy pontra
		...
		force = force + (-graph->vertices[i] * 3.0f);//globális erőtér
		force = force * 0.01f;
